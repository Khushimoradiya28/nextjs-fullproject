...
export default function MyPropertiesPage() {
  ...
  const handleMarkSold = async () => {
    setMarkingSold(true);
    const prop = properties.find((p) => p.id === soldId);
    const newStatus =
      prop?.status === "sold" || prop?.status === "inactive"
        ? "active"
        : "sold";
    const res = await updateProperty(soldId, { status: newStatus });
    setMarkingSold(false);
    if (res.success) {
      setProperties((p) =>
        p.map((item) =>
          item.id === soldId ? { ...item, status: newStatus } : item,
        ),
      );
      showWishlistToast(
        newStatus === "active"
      ...
      showWishlistToast("Failed to update status.", "removed");
    }
    setSoldId(null);
  };
  ...
  return (
    <div className={profileStyles.page}>
      ...
      <div className={profileStyles.pageContainer}>
        ...
        {!fetching && error && (
          <div className={styles.errorState}>
            ...
            <p>{error}</p>
            <button className={styles.retryBtn} onClick={fetchProperties}>
              Retry
            </button>
          </div>
        )}
                  ...
                  {paginatedProperties.map((prop) => (
                    <div key={prop.id} className={styles.propCard}>
                      ...
                      <div className={styles.propImage}>
                        <img
                          src={
                            prop.image && !prop.image.startsWith("blob:")
                              ? prop.image
                              : "/img/buy-properties/1.jpg"
                          }
                          alt={prop.title}
                          loading="lazy"
                        />
      ...
      {soldId && (
        <ConfirmModal
          title={
            properties.find((p) => p.id === soldId)?.status === "active"
              ? "Mark as Sold"
              : "Mark as Active"
          }
          message={
