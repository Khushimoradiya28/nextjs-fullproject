...
export default function BankPartnerDashboard() {
  ...
  useEffect(() => {
    if (!getToken()) {
      router.push("/bank-partner/login");
      return;
    }
    fetchProfile();
    fetchLeads();
  ...
  const loadNotifications = () => {
    setNotifications(JSON.parse(localStorage.getItem("bp_notifs") || "[]"));
  };
  const unreadCount = notifications.filter((n) => !n.read).length;
  const markAllRead = () => {
    const n = notifications.map((x) => ({ ...x, read: true }));
    setNotifications(n);
    localStorage.setItem("bp_notifs", JSON.stringify(n));
  };
  const showToast = (msg) => {
    setToast(msg);
  ...
  const fetchProfile = async () => {
    try {
      const r = await fetch(`${API}/bank-partners/profile`, {
        headers: authHeaders(),
      });
      if (!r.ok) return;
  ...
  const fetchLeads = async (status = "") => {
    ...
    try {
      const u = status
        ? `${API}/bank-partners/leads?status=${status}`
        : `${API}/bank-partners/leads`;
      ...
      const d = await r.json();
      if (d.success) setLeads(d.leads || d.data || []);
    } catch (e) {}
    setLeadsLoading(false);
  };
  ...
  const handleLogoUpload = async (e) => {
    ...
    const t = getToken();
    try {
      const r = await fetch(`${API}/bank-partners/profile/logo`, {
        method: "POST",
  ...
  const handleLeadUpdate = async (leadId, st, notes) => {
    try {
      const r = await fetch(`${API}/bank-partners/leads/${leadId}`, {
        method: "PATCH",
        headers: authHeaders(),
        body: JSON.stringify({ status: st, notes }),
      });
      const d = await r.json();
      if (d.success) {
        setLeads((prev) =>
          prev.map((l) => ((l._id || l.id) === leadId ? d.data || d.lead : l)),
        );
        const lead = leads.find((l) => (l._id || l.id) === leadId);
        addNotification(
          "Lead " + (lead?.name || "") + " marked as " + st.replace("_", " "),
        );
        loadNotifications();
      }
    } catch (e) {}
    ...
    >
      ...
      <div style={{ display: "flex", flex: 1 }}>
        ...
        <main style={{ flex: 1, padding: "28px 32px" }}>
          ...
          {(activeTab === "Profile & Offer" || activeTab === "Profile") && (
            <ProfileTab
              editForm={editForm}
          ...
          {/* Add Offer tab — form always open */}
          {activeTab === "Add Offer" && (
            <AllOffersTab
              offers={offers}
          ...
          {/* All Offers tab — form closed by default */}
